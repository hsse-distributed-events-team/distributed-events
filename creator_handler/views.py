...
def staff_list(request, event_id: int):
    if not c_db.user_have_access(request.user, event_id):
        return redirect('/404')
    staff = c_db.get_staff_by_event(event_id)
    context = {
        "navigation_buttons": NAVIGATE_BUTTONS,
        "staff_list": staff
    }
    return render(request, 'creator_handler/staff_list.html', context)


# @login_required
# def add_staff(request, event_id: int):
#     """
#     Страница добавления модераторов
#
